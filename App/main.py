...
# Fonction pour afficher les entrées du modèle de prédiction de rendement


def display_yield_prediction_inputs(predicted_seed):
    ...
    # Formulaire pour entrer les données du modèle de prédiction de rendement
    input_data = {
        'Area': st.selectbox('Choisir le pays', list_pays, key='pays'),
        'avg_temp': st.text_input('La température moyenne', key='temperature'),
    ...
    # Bouton pour lancer la prédiction de rendement
    if st.button('Faire une prédiction', key='predict_button'):
        df = pd.DataFrame([input_data])
        prediction = yield_prediction_model.predict(df)

        # Afficher le résultat de la prédiction
        st.success("Prédiction du rendement")
        st.write(f"Vous aurez une production de {
                 prediction[0]} Hectogrammes/Hectare si vous cultivez {predicted_seed}")
...
if st.button('Predict Crop Type'):
    ...
    # Stocker la semence prédit dans l'état de session pour l'utiliser plus tard
    st.session_state.predicted_seed = predicted_seed

    # Bouton pour afficher les entrées du deuxième modèle
    if st.button("Voulez-vous voir la prédiction du rendement ?"):
        if 'predicted_seed' in st.session_state:
            display_yield_prediction_inputs(st.session_state.predicted_seed)
        else:
            st.warning("Veuillez d'abord prédire le type de culture.")
